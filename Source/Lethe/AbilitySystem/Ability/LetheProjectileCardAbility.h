...
/**
 * Projectile을 발사하는 Ability로, EffectTargetMappingPolicy를 그대로 따릅니다.
...
protected:
	virtual void ExecuteEffectAppliersByPolicy(const FEffectTargetMappingPolicy& EffectTargetMappingPolicy, AActor* TargetActor) override;
	
	/**
	 * Projectile을 생성 및 발사하는 함수입니다.
	 * @param InProjectileSpawnLocation Projectile이 생성될 위치입니다.
	 * @param EffectTargetMappingPolicy 어떤 대상에게 어떤 EffectApplier를 적용할지 결정하는 정책 묶음으로, 여기선 EffectApplier를 가져오기 위해 사용합니다.
	 * @param TargetActor Projectile의 목표 대상입니다.
	 */
	void SpawnProjectiles(const FVector& InProjectileSpawnLocation, const FEffectTargetMappingPolicy& EffectTargetMappingPolicy, AActor* TargetActor) const;
