...
void AArrowRenderer::SetPoints(const FVector& StartLocation, const FVector& EndLocation) const
{
	...
	const FRotator HeadRotation = Spline->GetRotationAtDistanceAlongSpline(Spline->GetSplineLength(), ESplineCoordinateSpace::World);
	ArrowHead->SetWorldLocation(MeshEnd);
	ArrowHead->SetRelativeRotation(HeadRotation);
