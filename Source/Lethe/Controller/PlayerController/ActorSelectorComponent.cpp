...
bool UActorSelectorComponent::TryGetPlayerCharacterMovableTiles(TArray<ATile*>& OutTiles, const AActor* PlayerCharacter, const FBFSRange& MoveRange) const
{
	if (UTileManagerSubsystem* TileManagerSubsystem = GetWorld()->GetSubsystem<UTileManagerSubsystem>())
	{
		if (const ATile* Tile = TileManagerSubsystem->GetTileUnderActor(PlayerCharacter))
		{
			TSet<FCubeCoord> TilesInRange;
			const FCubeCoord ThisTileCoord = Tile->GetCubeCoord();

			const auto CanMoveToTile = [&TileManagerSubsystem](const FTileData* TileData) -> bool
			{
				if (TileData && TileData->TileActor.IsValid())
				{
					return TileManagerSubsystem->CanMoveToTileForPlayerCharacter(TileData->TileActor.Get());
				}
				return false;
			};
			
			TileManagerSubsystem->TileBFS(ThisTileCoord, MoveRange.Distance, MoveRange.BFSType, TilesInRange,
			[&CanMoveToTile](const FTileData* CurrentTileData, const FTileData* NextTileData)
			{
				// 이동 불가능한 타일은 그 타일을 거치는 경로도 제외되도록 합니다.
				return CanMoveToTile(NextTileData);
			},
			[&CanMoveToTile, &OutTiles](const FCubeCoord CurrentCoord, const FTileData* TileData, const int32 Depth)
			{
				if (CanMoveToTile(TileData))
				{
					OutTiles.Emplace(TileData->TileActor.Get());
					return true;
				}
				return false;
			});
